...
class Spectator {
    ...
    public function getCurrentGameInfo($region, $encryptedSummonerId)
    {
        try{
            $this->api->setRegion($region);
            return new Response(json_encode($this->api->getCurrentGameInfo($encryptedSummonerId)));
        } catch (Exception $e) {
    ...
    public function getFeaturedGames($region)
    {
        try{
            $this->api->setRegion($region);
            return new Response(json_encode($this->api->getFeaturedGames()));
        } catch (Exception $e) {
            return new Response(json_encode(array(
                "status" => array(
                    "status_code" => $e->getCode(),
